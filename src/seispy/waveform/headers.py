"""Populate SAC headers from station and event metadata."""
...
class _EventSummary:
    ...
    failed: int = 0
    conflicts: int = 0
    samples: tuple[SacHeaderIssue, ...] = ()


@dataclass(frozen=True)
class SacHeaderSummary(BatchSummary):
    """Summarize a batch SAC header-formatting run.

    This class is returned by :func:`format_sac_headers`; applications normally do not
    ...
    Attributes:
        ...
        issue_samples: Bounded sample of formatting issues.
    ...
    issue_samples: tuple[SacHeaderIssue, ...]
    output_dir: Path
...
def format_sac_headers(
    ...
    *,
    ...
    if events["event_dir"].duplicated().any():
        raise ValueError(
            "events_csv contains duplicate event times at one-second precision"
        )
    ...
    tasks = [
        (path, events_dict[path.name])
        for path in event_dirs
        if path.name in events_dict
    ]
    ...
    with BatchRun(
        ...
        run.info(
            "run_id=%s src=%s dest=%s pattern=%s overwrite=%s max_workers=%d",
            run_id,
            src_path,
            dest_path,
            pattern,
        ...
        if summary.failed > len(summary.issue_samples):
            run.warning(
                "run_id=%s error_samples_truncated shown=%d total=%d",
                run_id,
                len(summary.issue_samples),
                summary.failed,
            )
    print(
        f"SAC formatting [{run_id}]: {summary.succeeded} succeeded, "
        f"{summary.failed} failed, {summary.events_skipped} events skipped."
    )
    return summary


def _run_format_tasks(
    tasks,
    dest_path,
    pattern,
    stations_dict,
        ...
        with progress_bar(
            ...
            for future in as_completed(futures):
                ...
                run.checkpoint(
                    ...
                    events_processed=len(summaries),
                    events_skipped=events_skipped,
                    invalid_event_times=invalid_times,
                    files_processed=combined.total,
                    succeeded=combined.succeeded,
                    failed=combined.failed,
                    output_conflicts=combined.conflicts,
                    issue_samples=combined.samples,
                ...
                bar.update(1)
    return _combine(summaries, max_error_samples)
...
def format_per_event(
    ...
    for source in files:
        ...
        try:
            ...
            destination = (
                dest_path / event_dir.name / f"{event_dir.name}.{station}.{channel}.sac"
            )
            ...
            if len(samples) < max_error_samples:
                samples.append(
                    SacHeaderIssue(
                        source,
                        "output_conflict" if conflict else "format_failed",
...
def _failed_event(files, exc, limit):
    samples = (
        (SacHeaderIssue(files[0], "event_failed", f"{type(exc).__name__}: {exc}"),)
        if files and limit
        else ()
    )
