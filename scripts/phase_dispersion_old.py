...
# BASE_DIR = Path("data/lyb_data")
# ANT_DIR = BASE_DIR / "ant_grids"
# TPWT_DIR = BASE_DIR / "tpwt_grids"

# OUT_FILE = Path("data/mcmc/phase_dispersion_lyb.csv")


# def parse_period_from_name(path: Path, prefix: str, suffix: str = ".grid") -> float:
#     pattern = rf"{prefix}_(\d+(?:\.\d+)?)\{suffix}$"
...
# def read_grid_file(path: Path, value_name: str) -> pd.DataFrame:
#     df = pd.read_csv(
#         path,
#         sep=r"\s+",
#         header=None,
...
#     df = df[["longitude", "latitude", value_name]].copy()

#     for col in df.columns:
#         df[col] = pd.to_numeric(df[col], errors="coerce")

#     df = df.dropna(subset=["longitude", "latitude", value_name])

#     return df


# def read_ant_grids(ant_dir: Path) -> pd.DataFrame:
#     rows = []

#     for file in sorted(ant_dir.glob("ant_vel_*.grid")):
#         period = parse_period_from_name(file, prefix="ant_vel")

#         df = read_grid_file(file, "phv")
#         df["period"] = period
#         df["source"] = "ANT"
...
#     return ant[["period", "longitude", "latitude", "phv", "source"]]


# def read_tpwt_grids(tpwt_dir: Path) -> pd.DataFrame:
#     vel_rows = []

#     for vel_file in sorted(tpwt_dir.glob("tpwt_vel_*.grid")):
#         period = parse_period_from_name(vel_file, prefix="tpwt_vel")

#         vel = read_grid_file(vel_file, "phv")
#         vel["period"] = period

#         std_file = tpwt_dir / f"tpwt_std_{period:g}.grid"

#         if std_file.exists():
#             std = read_grid_file(std_file, "std")
#             std["period"] = period
...
#             df = vel.copy()
#             df["std"] = np.nan
#             print(f"[WARN] Missing std file for period={period:g}: {std_file}")

#         df["source"] = "TPWT"
#         vel_rows.append(df)

#     if not vel_rows:
#         raise FileNotFoundError(f"No TPWT velocity grid files found in {tpwt_dir}")

#     tpwt = pd.concat(vel_rows, ignore_index=True)

#     return tpwt[["period", "longitude", "latitude", "phv", "std", "source"]]


# def merge_ant_tpwt(ant: pd.DataFrame, tpwt: pd.DataFrame) -> pd.DataFrame:
...
#         phv_ant = r.get("phv_ant", np.nan)
#         phv_tpwt = r.get("phv_tpwt", np.nan)
#         std_tpwt = r.get("std_tpwt", np.nan)

#         has_ant = np.isfinite(phv_ant)
#         has_tpwt = np.isfinite(phv_tpwt)

#         if has_ant and has_tpwt:
#             phv = float(np.mean([phv_ant, phv_tpwt]))
...
# def main():
#     ant = read_ant_grids(ANT_DIR)
#     tpwt = read_tpwt_grids(TPWT_DIR)

#     phase = merge_ant_tpwt(ant, tpwt)
...
OUT_FILE = Path("data/mcmc/phase_dispersion_nz.csv")
