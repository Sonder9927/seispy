    ...
    with tqdm(total=total, desc="Processing...") as pbar:
        for station in stations:
            for event in events:
                ...
                pbar.update(1)
    logging.info("Cut events complete.")
    ...
    for sac_path in sac_files:
        try:
            ...
            if sta != station["station"]:
                logging.error(f"File name mismatch: {sac_path}.")
                continue
            ...
            logging.error(f"Error processing file {sac_path}: {e}")
    ...
    if channel_data:
        ...
        for channel, stream in channel_data.items():
            try:
                ...
                logging.error(f"{event_name} {channel} failed: {e}")
...
def _load_stations(src_dir, station_csv: str | None) -> list[dict]:
    """
    load stations from src_dir or station_csv
    """
...
def cut_events_bin(
    ...
    # binarary
    mktraceiodb = "bin/mktraceiodb"
    cutevent = "bin/cutevent"
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO, filename="cut_event.log")
    cut_events("data_sac", "data_events", "events.csv", "stations.csv", 10800)
    cut_events_bin("data_sac", "data_events", "events.cat", 10800)
