...
from seispy.download import mass
...
def test_mass_download_builds_daily_flat_storage_and_restrictions(tmp_path):
    ...
    domain = object()
    with patch.object(mass, "MassDownloader", return_value=downloader) as factory:
        result = mass.download_waveforms_mass(
            tmp_path / "waveforms",
            "2025-01-01",
            "2025-01-03",
            domain=domain,
...
def test_mass_download_defaults_to_global_domain(tmp_path):
    downloader = Mock()
    with (
        patch.object(mass, "MassDownloader", return_value=downloader),
        patch.object(mass, "GlobalDomain", return_value="global") as global_domain,
    ):
        mass.download_waveforms_mass(tmp_path, "2025-01-01", "2025-01-02", network="NZ")
...
def test_mass_download_uses_supplied_inventory_as_station_limit(tmp_path):
    ...
    with patch.object(mass, "MassDownloader", return_value=downloader):
        mass.download_waveforms_mass(
            tmp_path,
            "2025-01-01",
            "2025-01-02",
            inventory=inventory,
...
def test_mass_download_reads_stationxml_inventory(tmp_path):
    ...
    with (
        patch.object(mass, "MassDownloader", return_value=downloader),
        patch.object(mass, "read_inventory", return_value=inventory) as read,
    ):
        mass.download_waveforms_mass(
            tmp_path / "output",
            "2025-01-01",
...
def test_mass_download_validates_options_before_creating_downloader(
    ...
    with (
        patch.object(mass, "MassDownloader") as downloader,
        pytest.raises(ValueError, match=message),
    ):
        mass.download_waveforms_mass(tmp_path, "2025-01-01", "2025-01-02", **kwargs)
