"""Waveform-download availability contracts."""
...
def test_plot_does_not_inherit_an_unavailable_global_font(tmp_path):
    _waveform(tmp_path, "AAA", 2024, 1)
    availability = scan_download_availability(tmp_path / "NZ")

    with mpl.rc_context({"font.family": ["Unavailable Custom Font"]}):
        figure, axes = plot_download_availability(availability)
    ...
    assert {tuple(text.get_fontfamily()) for text in text_artists} == {("DejaVu Sans",)}
    plt.close(figure)
...
def test_download_status_exports_combined_figure_and_summary(tmp_path):
    ...
    assert isinstance(report, DownloadAvailabilityReport)
